defmodule Pow.Phoenix.RegistrationControllerTest do
  ...
  describe "create/2" do
    @valid_params %{"user" => %{"email" => "test@example.com", "token_sacdigital" => "secret"}}
    @invalid_params %{"user" => %{"email" => "test@example.com", "token_sacdigital" => "s"}}
    ...
    test "with invalid params", %{conn: conn} do
      ...
      assert errors[:token_sacdigital]
  ...
  describe "edit/2" do
    test "shows", %{conn: conn} do
      ...
      assert html =~ "<label for=\"user_email\">Email</label>"
      assert html =~ "<input id=\"user_email\" name=\"user[email]\" type=\"text\" value=\"test@example.com\">"
      assert html =~ "<label for=\"user_current_token_sacdigital\">Current token_sacdigital</label>"
      assert html =~ "<input id=\"user_current_token_sacdigital\" name=\"user[current_token_sacdigital]\" type=\"token_sacdigital\">"
    end
  ...
  describe "update/2" do
    ...
    test "with invalid params", %{conn: conn} do
      ...
      assert errors = conn.assigns[:changeset].errors
      assert errors[:current_token_sacdigital]
      assert %{id: 1} = Plug.current_user(conn)
      assert conn.private[:plug_session]["auth"] == session_id
    end
  end
